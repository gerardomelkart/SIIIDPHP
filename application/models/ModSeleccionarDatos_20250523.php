...
class ModSeleccionarDatos extends CI_Model{
	...
	public function obtenConsultaLogin($usuario,$contrasena){

		return $this->db->query("SELECT * FROM usuarios WHERE USUARIO='$usuario' and PASSWORD='$contrasena' and VIGENTE = 1 ")->result();
    ...
    public function ValidarIntegridadDatos($codigoReferencia){
    	...
    	// return $this->db->query("SELECT carp.id_ci
		// 							FROM tmp_carpetas carp
		// 							LEFT JOIN tmp_delitos deli ON carp.id_ci = deli.id_ci
		// 							LEFT JOIN tmp_victimas vic ON carp.id_ci = vic.id_ci
		// 							WHERE (deli.id_ci IS NULL OR vic.id_ci IS NULL) AND carp.codigo_referencia = '$codigoReferencia';")->result();


    	// return $this->db->query("SELECT 
		// 						  c.id_ci AS 'ID Carpeta',
		// 						  CASE
		// 						    WHEN d.id_delito IS NULL THEN 'Sin delitos'
		// 						    WHEN v.id_vicf IS NULL THEN 'Sin víctimas'
		// 						  END AS 'Estado'
		// 						FROM tmp_carpetas c
		...
		// 						  ON c.id_ci = v.id_ci 
		// 						  AND d.id_delito = v.id_delito  -- Relación entre delitos y víctimas
		// 						WHERE d.id_delito IS NULL         -- Carpeta sin delitos
		// 						  OR v.id_vicf IS NULL
		// 						AND c.codigo_referencia = '$codigoReferencia'; ")->result();


		return $this->db->query("SELECT  c.id_ci as id_ci_inconsistente from tmp_carpetas c
								where ( not exists (select * from tmp_delitos d
													where c.id_ci = d.id_ci)
								         or not exists (select * from tmp_victimas v
												    where c.id_ci = v.id_ci) ) and c.codigo_referencia = '$codigoReferencia'  
							union 

							SELECT d.id_ci as id_ci_inconsistente from tmp_delitos d
								where ( not exists (select * from tmp_carpetas c
													where d.id_ci = c.id_ci)  
								        or not exists (select * from tmp_victimas v
													where d.id_ci = v.id_ci and d.id_delito = v.id_delito) ) and d.codigo_referencia = '$codigoReferencia' 
							union

							SELECT v.id_ci as id_ci_inconsistente from tmp_victimas v
								where  ( not exists (select * from tmp_carpetas c
														where v.id_ci = c.id_ci)							
								        or not exists (select * from tmp_delitos d
													where v.id_ci = d.id_ci and v.id_delito = d.id_delito) ) and v.codigo_referencia = '$codigoReferencia' ")->result();
    ...
    public function ValidarDatosArchCarpetasInv($codigoReferencia, $proceso){

    	    // Ejecutar el procedimiento almacenado
		    $query = $this->db->query("CALL sp_ValidarDatosArchCarpetas('$codigoReferencia', '$proceso')");
    ...
    public function ConsultaConstTablaAcuseRecibo($codigo,$proceso){
	    	...
	    	if($proceso == 'CargaInfo'){
	    		...
	    		return $this->db->query("SELECT 
										    ...
										    (SELECT COUNT(DISTINCT id_ci, id_delito) 
										     FROM tbl_delitos 
    ...
    public function ConsultaDelitosConstTablaAcuseRecibo($codigo){
    	...
    	return $this->db->query("SELECT cd.clave2,
    									cd.delito,  
									    cd.clave3,
									    cd.subtipo_delito,
									    COUNT(DISTINCT td.pk_del) AS tot_delitos,
									    COUNT(DISTINCT tv.pk_vict) AS tot_victimas  
									FROM cat_delitos cd
									LEFT JOIN tbl_delitos td 
									    ON cd.clave4 = td.clasf_de_dto 
									    AND td.codigo_referencia = '$codigo'
									LEFT JOIN tbl_victimas tv 
									    ON td.pk_ci = tv.pk_ci 
									    AND td.pk_del = tv.pk_del 
									    AND td.codigo_referencia = '$codigo'
									GROUP BY cd.clave3, cd.subtipo_delito
									ORDER BY cd.clave3;")->result();
...
/*******************************VALIDACIONES DELITOS********************************************/
    	...
    	if ($inconsistencia == 'ID_DELITO_duplicados'){
    		...
    		return $this->db->query("SELECT id_ci, id_delito, COUNT(*) AS duplicados
										FROM tmp_delitos
										where codigo_referencia = '$codRef'
    	...
    	if ($inconsistencia == 'fcha_de_hchos_formatoIncorrecto'){
    		...
    		return $this->db->query("SELECT id_ci, id_delito, dto, fha_de_hchos FROM tmp_delitos
										WHERE fha_de_hchos NOT REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
										AND codigo_referencia = '$codRef' ")->result();
    	...
    	if ($inconsistencia == 'cve_entFed_incorrecta'){
    		...
    		return $this->db->query("SELECT t1.id_ci, t1.id_delito, t1.dto, t1.id_ent_hchos FROM tmp_delitos t1
										 WHERE NOT EXISTS (SELECT * FROM cat_entidades t2
    	...
    	if ($inconsistencia == 'coord_x_fmato_incorrecto'){
    		...
    		return $this->db->query("SELECT id_ci, id_delito, dto, coord_x FROM tmp_delitos
										WHERE coord_x NOT REGEXP '^[+-]?([0-9]+(\\.[0-9]+)?|\\.[0-9]+)$' 
										AND codigo_referencia = '$codRef' ")->result();
    	}
    	...
    	if ($inconsistencia == 'coord_y_fmato_incorrecto'){
    		...
    		return $this->db->query("SELECT id_ci, id_delito, dto, coord_y FROM tmp_delitos
										WHERE coord_x NOT REGEXP '^[+-]?([0-9]+(\\.[0-9]+)?|\\.[0-9]+)$' 
										AND codigo_referencia = '$codRef' ")->result();
    	}
    	...
    	if ($inconsistencia == 'dato_poblacion_incorrecto'){
    		...
    		return $this->db->query("SELECT t1.id_ci, t1.id_delito, t1.id_tv, t1.pob FROM tmp_victimas t1
										WHERE t1.pob NOT IN (0, 1) AND t1.id_tv = 1 
										AND t1.codigo_referencia = '$codRef'")->result();
    	}
    	...
    	if ($inconsistencia == 'dato_discapacidad_incorrecto'){
    		...
    		return $this->db->query("SELECT t1.id_ci, t1.id_delito, t1.id_tv, t1.disc FROM tmp_victimas t1
										WHERE t1.disc NOT IN (0, 1) AND t1.id_tv = 1 
										AND t1.codigo_referencia = '$codRef'")->result();
    	}
    	...
    	if ($inconsistencia == 'fmto_fchaNac_incorrecto'){
    		...
    		return $this->db->query("SELECT t1.id_ci, t1.id_delito, t1.id_tv, t1.fha_nac FROM tmp_victimas t1
										WHERE fha_nac NOT REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' AND t1.id_tv = 1
										AND codigo_referencia = '$codRef' ")->result();
    	}
    ...
    public function obtRegExpedientesEnviados($codRef){
    	...
    	return $this->db->query("SELECT id_ci, ntra_ci, fha_de_ini, hra_de_ini, id_texp, rmen_de_hchos, usuario_reg as usuario_envio, date(fcha_insert) as fecha_envio FROM tbl_carpetas WHERE codigo_referencia = '$codRef'")->result();

    }

    public function obtRegDelitosEnviados($codRef){

		return $this->db->query("SELECT id_ci, id_delito, dto, dto_prin, moda_dto, forma_acc, fha_de_hchos, hra_de_hchos, emto_com_dto, grdo_cons, clasf_de_dto, nom_ent_hchos, id_ent_hchos, nom_mun_hchos, id_mun_hchos, nom_loc_hchos, id_loc_hchos, nom_col_hchos, id_col_hchos, cp, coord_x, coord_y, dom_hchos, usuario_reg as usuario_envio, date(fcha_insert) as fecha_envio
    ...
    public function obtRegVictimasEnviados($codRef){
	    ...
	    return $this->db->query("SELECT id_ci, id_delito, id_vicf, id_tv, id_tpm, sexo, genero, pob, disc, fha_nac, edad, nacional, rel_vic_vmario, usuario_reg as usuario_envio, date(fcha_insert) as fecha_envio 
